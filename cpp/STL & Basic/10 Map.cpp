...
int main(){
    map<int, int> a;
    a.insert({1,2});  // Cách tạo key và value mặc định
    a[3] = 4;        
    a[5];           // Nếu key đó chưa có thì tự thêm Key đó vào map với value = 0
...
cout << "Duyệt bằng Range Base For Loop: ";
for( auto i : a ){
...
==> Làm điều kiện nếu chưa có trong set/map thì thêm vô Vector 